...
class KanaPluginSystem:
    ...
    def discover_plugins(self):
        ...
        core_plugins = {
            ...
            "brain_builder": {
                "module": "brain_builder",
                "class": "KanaBrainBuilder",
                "description": "AI learning memory builder",
                "requires": []
            },
            "death_tracker": {
                "module": "death_wallet_tracker",
                "class": "DeathWalletTracker",
                "description": "Founder wallet tracking",
                "requires": []
            },
            "repo_tools": {
                "module": "repo_tool_loader",
                "class": "RepoToolLoader",
                "description": "Auto-load tools from repos",
                "requires": []
            }
        }
...
class KanaOmniHybrid:
    ...
    def __init__(self):
        ...
        self._immunefi = None
        self._analyzer = None
        self._brain_builder = None
        self._death_tracker = None
    ...
    @property
    def analyzer(self):
        """Lazy load analyzer plugin"""
        if self._analyzer is None:
            self._analyzer, _ = self.plugin_system.load_plugin("analyzer")
        return self._analyzer

    @property
    def brain_builder(self):
        """Lazy load brain builder plugin"""
        if self._brain_builder is None:
            self._brain_builder, _ = self.plugin_system.load_plugin("brain_builder")
        return self._brain_builder

    @property
    def death_tracker(self):
        """Lazy load death tracker plugin"""
        if self._death_tracker is None:
            self._death_tracker, _ = self.plugin_system.load_plugin("death_tracker")
        return self._death_tracker

    @property
    def repo_tools(self):
        """Lazy load repo tools plugin"""
        if self._repo_tools is None:
            self._repo_tools, _ = self.plugin_system.load_plugin("repo_tools")
        return self._repo_tools
    ...
    def show_menu(self):
        ...
        print("[6] 📊 Cuan Analyzer - Security analysis")
        print("[7] 🧠 Brain Builder - AI learning memory")
        print("[8] 💀 Death Wallet Tracker - Founder wallets")
        print("[9] 🔧 Repo Tool Loader - Auto-load tools")
        print("[10] 🛡️ White Hat Worker - Dead address scanner")
        print("[11] 🐛 Immunefi Worker - Bug bounty automation")
        print("[12] 📋 Combined Workflow - Full pipeline")
        print("[13] 📋 Tool Status - Available tools")
        print("[14] ⚙️ Settings - Configuration")
        print("[0] 🚪 Exit\n")
    ...
    def menu_analyzer(self):
        if not self.analyzer:
            print("[!] Cuan Analyzer plugin not available")
            return

        print("[*] Cuan Analyzer would analyze vulnerabilities here...")
        print("[*] (Full implementation in separate plugin file)")

    def menu_brain_builder(self):
        if not self.brain_builder:
            print("[!] Brain Builder plugin not available")
            return

        print("[*] Brain Builder would train AI here...")
        print("[*] (Full implementation in separate plugin file)")

    def menu_death_tracker(self):
        if not self.death_tracker:
            print("[!] Death Wallet Tracker plugin not available")
            return

        print("[*] Death Wallet Tracker would monitor wallets here...")
        print("[*] (Full implementation in separate plugin file)")

    def menu_repo_tools(self):
        if not self.repo_tools:
            print("[!] Repo Tool Loader plugin not available")
            return

        print("[*] Repo Tool Loader would discover tools here...")
        print("[*] (Full implementation in separate plugin file)")
    ...
    def menu_combined(self):
        print("\n=== 📋 COMBINED WORKFLOW ===")
        print("[*] Run integrated: Inject → Scrape → Analyze → Hunt\n")

        # Step 1: Inject
        print("[1/4] Repository Injection")
        link = input("[?] GitHub Repo URL (or 'skip'): ").strip()
        if link and link.lower() != 'skip':
            if "github.com" in link and not link.endswith(".git"):
                link += ".git"
            self.injector.process_repo(link)

        # Step 2: Scrape
        print("\n[2/4] Content Scraping")
        content_input = input("[?] Paste URL/Content (or 'skip'): ").strip()
        if content_input and content_input.lower() != 'skip':
            content, source = self.scraper.scrape_content(content_input)
            if content:
                scraped_path, _ = self.scraper.save_scraped_content(content, source)
                print(f"[+] Content saved: {scraped_path}")

        # Step 3: Analyze (if available)
        if self.analyzer:
            print("\n[3/4] Vulnerability Analysis")
            analyze_choice = input("[?] Run analyzer? (y/n): ").strip().lower()
            if analyze_choice == 'y':
                print("[*] Running analyzer...")
        else:
            print("\n[3/4] Vulnerability Analysis - Plugin not available")

        # Step 4: Hunt (placeholder)
        print("\n[4/4] Pattern Hunting")
        hunt_choice = input("[?] Run hunt? (y/n): ").strip().lower()
        if hunt_choice == 'y':
    ...
    def menu_tool_status(self):
        ...
        for plugin_name, status in plugin_status.items():
            ...
            print(f"{icon} {plugin_name.title().replace('_', ' ')} - {status['description']}{loaded}")

    def menu_settings(self):
        print("\n=== ⚙️ SETTINGS ===")
        print("[1] Check GitHub Token")
        print("[2] View knowledge_base path")
        print("[3] List scraped files")
        print("[4] View plugin status")
        ...
        if choice == '1':
            ...
            print(f"[+] knowledge_base path: {self.scraper.kb_path}")

        elif choice == '3':
            scraped = list(self.scraper.scrape_dir.glob('scraped_*'))
            if scraped:
                print(f"\n[+] Found {len(scraped)} scraped files:")
                for f in scraped[:10]:
                    print(f"   - {f.name}")
            else:
    ...
    def run(self):
        ...
        while True:
            ...
            if choice == '1':
                ...
                self.menu_brain_builder()
            elif choice == '8':
                self.menu_death_tracker()
            elif choice == '9':
                self.menu_repo_tools()
            elif choice == '10':
                self.menu_whitehat()
            elif choice == '11':
                self.menu_immunefi()
